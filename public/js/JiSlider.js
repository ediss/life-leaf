...
(function ($) {
	$.fn.JiSlider = function (options) {
		...
		if (setting.controller) {
			var leftArrow = $('<div>', {
				'class': 'jislider__left-arrow'
			}).click(function () {
				animate.control(--animate.index);
			});
			var leftArrowTop = $('<div>', {
				'class': 'jislider__left-arrow__top'
			}).css({
				backgroundColor: setting.color,
			});
			var leftArrowBottom = $('<div>', {
				'class': 'jislider__left-arrow__bottom'
			}).css({
				backgroundColor: setting.color,
			});

			var rightArrow = $('<div>', {
				'class': 'jislider__right-arrow'
			}).click(function () {
				animate.control(++animate.index);
			});
			var rightArrowTop = $('<div>', {
				'class': 'jislider__right-arrow__top'
			}).css({
				backgroundColor: setting.color,
			});
			var rightArrowBottom = $('<div>', {
				'class': 'jislider__right-arrow__bottom'
			}).css({
				backgroundColor: setting.color,
			});

			var controller = $('<div>', {
				'class': 'jislider__controller'
			}).css({
				width: 20 * slides,
			});
